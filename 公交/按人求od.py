...
for mm in range(yue,yue+1):		
	for dd in range(7,8):	#日区间
		...
		try:
			...
			with open('D://广州//公交数据//公交刷卡数据//shuchu//'+str(int(mm)+1)+'-'+str(1)+'//chenggong.csv','r+',encoding='UTF-8')as f:
				for hang in f.readlines():
					n+=1
					if n%50000==0:
						print(str(mm),str(dd),':',n)
		...
		if os.path.exists('E:\广州公交\yjt\人_od\\'+str(mm)+str(dd)) == False:
			os.makedirs('E:\广州公交\yjt\人_od\\'+str(mm)+str(dd)+'/')
